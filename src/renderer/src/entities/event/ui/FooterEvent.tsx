    ...
    return (
        <section className="bg-primary relative flex-1 rounded-xl p-4">
            <div className="mb-2 flex items-center justify-between">
                <h3 className="text-primary items-center font-semibold">{title}</h3>

                <div className="flex flex-row">
                    <Button onClick={handleScrollUp} disabled={!canScrollUp} variant="ghost" size="icon">
                        <ChevronUp />
                    </Button>
                    <Button onClick={handleScrollDown} disabled={!canScrollDown} variant="ghost" size="icon">
                        <ChevronDown />
                    </Button>
                </div>
            </div>
                    ...
                    visibleItems.map((event, i) => (
                        <div key={visibleStartIndex + i} className="flex items-center gap-3 px-3">
                            ...
                            <div className="flex flex-col">
                                <span className="text-primary font-semibold">{event.summary}</span>
                                <span className="text-font-gray text-sm">{formatKoreanDateTime(event.start.dateTime)}</span>
                            </div>
                        </div>
                    ))
                )}
